 ...
 * This class will listen to the local database and send all items with Send
 */
public class DatabaseSyncListenerService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent.getStringExtra(CHAT_SYNC_UID_USER) != null && !intent.getStringExtra(CHAT_SYNC_UID_USER).isEmpty()) {
            ...
            disposableChatByStatus = chatRepository.findFlowableByUidUserAndStatusNotIn(uidUser, Utility.allStatusToSend())
                    .subscribeOn(Schedulers.io()).observeOn(Schedulers.io())
                    .doOnError(e -> Log.e(TAG, e.getLocalizedMessage(), e))
                    .doOnNext(coreSync::sendNewChat)
                    .subscribe();

            disposableMessageByStatus = messageRepository.findFlowableByStatus(Utility.allStatusToSend())
                    .subscribeOn(Schedulers.io()).observeOn(Schedulers.io())
                    .doOnError(e -> Log.e(TAG, e.getLocalizedMessage(), e))
